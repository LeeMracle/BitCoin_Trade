...
| 목표 | WBS ID | 상태 | 비고 |
|------|--------|------|------|
| 주말(04-19~20) 서버 상태 점검 | (운영) | 진행 | systemctl active + heartbeat + 포지션 변동 + regime_check 로그 |
| 필터 통계 3일치 확인 (filter_stats.json) | (운영) | 진행 | P7-09/10 실측 누적 검증 |
| 레짐 상태 확인 (regime_state.json) | (운영) | 진행 | BEAR/SIDEWAYS 판정 + 히스테리시스 카운트 |
| P5-02/03 트리거 조건 점검 | P5-02/03 | 대기 | BULL 레짐 7일 유지 시 자동 착수 |
| lint_history --weekly 주간 집계 | P6-13 후속 | 스트레치 | 일요일(04-26) 실행 권장 |
| VB 샘플 수집 모니터링 | P5-28b 후속 | 조건부 | vb_recheck_trigger cron 09:15 자동 |
| P5-04 LIVE 승격 ADR 초안 | P5-04 후속 | 스트레치 | 현재 DRY-RUN(enabled=false) |
| 잔고 로그 throttle 회귀 점검 | - | 대기 | 3일치 log_volume 임계 초과 여부 |
