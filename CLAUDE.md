...
**[workspace/reports/20260824_session_handoff.md](workspace/reports/20260824_session_handoff.md)**

직전 세션(2026-08-22~24) 인수인계. 아래를 담고 있다:

- **다음 작업: 5.5% 익절 전략** — 착수 전 §4 필독.
  08-23 측정에서 TP1을 늦출수록 성과가 좋았다(현행 5% → OOS +0.7% / 15% → +10.8%).
  5.5%는 현행과 같은 구간이라 개선 효과가 낮을 것으로 예상되며, 근거를 사용자에게 먼저 제시할 것.
  "전량 익절"인지 "부분 익절"인지 **의도 확인 필요**.
- **⚠ 선행 필수: 유령 포지션 버그(§3)** — TP 전 단계 완료 시 포지션이 제거되지 않아
  슬롯을 영구 점유하고 `closed_trades`에 기록되지 않는다. 5.5% 전량 익절이면
  승리 거래마다 재발하므로 **반드시 먼저 수정**.
- 이번 세션 처리분(버그 #40~#44, ADR 20260823-1/2), 미해결 목록, 반복 함정
