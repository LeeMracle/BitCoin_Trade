...
**[workspace/reports/20260824_2_session_handoff.md](workspace/reports/20260824_2_session_handoff.md)**

직전 세션(2026-08-24) 인수인계. 아래를 담고 있다:

- 🔴 **다음 작업: 슬롯 수 확대 (5 → 10 또는 15)** — 착수 전 §2 필독.
  측정상 슬롯은 TP보다 훨씬 강한 축이고 **모든 지표가 단조 개선**된다
  (슬롯5 IS -12.5%/OOS +2.4% → 슬롯15 +3.2%/+14.9%, 승률·MDD·변동성도 함께 개선).
  단 ⚠ **생존편향이 슬롯 확대에 유리하게 작용**하므로 크기는 과대평가 가능 —
  슬롯 20 확정은 위험. 최소주문 마진(슬롯20은 자본 210,500원 필요, 현재 264,000원)도 확인할 것.
  **시드머니 증액은 불필요** — 수익률(%)은 자본과 무관함을 300k vs 1M 측정으로 확인
- **처리 완료**: 유령 포지션 버그(lessons #45), TP1 +5%→+5.5%(ADR 20260824-1)
- **⚠ 다음 세션이 먼저 알아야 할 것 (§3)** — TP 측정 결과 세 가지:
  (1) 5.5%는 기존 5%와 **통계적으로 구분되지 않는다**(1σ 이내, IS는 방향 반대) —
  현행 설정은 "개선"이 아니라 "중립"이다,
  (2) TP1을 늦출수록 크게 개선된다(15% 전량 OOS +17.0%, 08-23 탐색과 독립적으로 일치),
  (3) **어느 설정도 IS 양수가 아니다** — TP 조정만으로 ADR 20260823-2는 미해결
- **§4 건드리지 말 것**: `services/ml/config.py` `LABEL_TARGET_PCT = 0.05` —
  TP와 0.5%p 어긋나지만 재학습 없이 바꾸면 threshold 0.45 보정이 깨진다
- **§6 반복 함정**: 검증 룰 역방향 테스트 의무, state 마이그레이션 시 배포 순서
  (봇 정지 → 배포 → 마이그레이션 → 기동), t3.micro 메모리 감시
- 미해결 목록

> 이전 인수인계(08-22~24 1차): [20260824_session_handoff.md](workspace/reports/20260824_session_handoff.md)
